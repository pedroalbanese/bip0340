...
func MarshalPublicKey(pub *PublicKey) ([]byte, error) {
	...
	if !ok {
		return nil, errors.New("bip0340: unsupported bip0340 curve")
	}
	...
	if !pub.Curve.IsOnCurve(pub.X, pub.Y) {
		return nil, errors.New("bip0340: invalid elliptic curve public key")
	}
...
func ParsePublicKey(derBytes []byte) (pub *PublicKey, err error) {
	...
	if err != nil {
		...
		err = errors.New("bip0340: trailing data after ASN.1 of public-key")
		return
	}
	...
	if !oid.Equal(oidPublicKeyBIP0340) {
		err = errors.New("bip0340: unknown public key algorithm")
		return
	}
	...
	if !paramsDer.ReadASN1ObjectIdentifier(namedCurveOID) {
		return nil, errors.New("bip0340: invalid parameters")
	}

	namedCurve := NamedCurveFromOid(*namedCurveOID)
	if namedCurve == nil {
		err = errors.New("bip0340: unsupported bip0340 curve")
		return
	}

	x, y := elliptic.Unmarshal(namedCurve, der)
	if x == nil {
		err = errors.New("bip0340: failed to unmarshal elliptic curve point")
		return
	}
...
func MarshalPrivateKey(key *PrivateKey) ([]byte, error) {
	...
	if err != nil {
		return nil, errors.New("bip0340: failed to marshal algo param: " + err.Error())
	}
	...
	if err != nil {
		return nil, errors.New("bip0340: failed to marshal EC private key while building PKCS#8: " + err.Error())
	}
...
func ParsePrivateKey(derBytes []byte) (*PrivateKey, error) {
	...
	if !privKey.Algo.Algorithm.Equal(oidPublicKeyBIP0340) {
		err = errors.New("bip0340: unknown private key algorithm")
		return nil, err
	}
	...
	if err != nil {
		return nil, errors.New("bip0340: failed to parse EC private key embedded in PKCS#8: " + err.Error())
	}
...
func marshalECPrivateKeyWithOID(key *PrivateKey, oid asn1.ObjectIdentifier) ([]byte, error) {
	if !key.Curve.IsOnCurve(key.X, key.Y) {
		return nil, errors.New("bip0340: invalid elliptic key public key")
	}
...
func parseECPrivateKey(namedCurveOID *asn1.ObjectIdentifier, der []byte) (key *PrivateKey, err error) {
	...
	if _, err := asn1.Unmarshal(der, &privKey); err != nil {
		return nil, errors.New("bip0340: failed to parse EC private key: " + err.Error())
	}

	if privKey.Version != ecPrivKeyVersion {
		return nil, fmt.Errorf("bip0340: unknown EC private key version %d", privKey.Version)
	}
	...
	if curve == nil {
		return nil, errors.New("bip0340: unknown elliptic curve")
	}
	...
	if k.Cmp(curveOrder) >= 0 {
		return nil, errors.New("bip0340: invalid elliptic curve private key value")
	}
	...
	for len(privKey.PrivateKey) > len(privateKey) {
		if privKey.PrivateKey[0] != 0 {
			return nil, errors.New("bip0340: invalid private key length")
		}
